---
name: release
description: Cut a cui-test-value-objects release — bump .github/project.yml version, open and merge the release PR, wait for the automated Release workflow, verify the release landed, then reformat the generated GitHub release notes
user-invocable: true
...
Cuts a new cui-test-value-objects release end-to-end: determine the version, open the version-bump PR
...
```yaml
consumers:
  - cuioss-parent-pom:version.cui.test.value.objects
  - nifi-extensions:version.cui.test.value.objects
```
...
```bash
gh pr list --repo cuioss/cui-test-value-objects --state open --json number,title,isDraft
```
- **No open PRs** → proceed.
...
if ! gh label list --repo cuioss/cui-test-value-objects --search skip-bot-review --json name --jq '.[].name' \
     | grep -qx skip-bot-review; then
  gh label create skip-bot-review --repo cuioss/cui-test-value-objects \
    --description "Skip automated bot review" --color ededed
fi
gh pr create --repo cuioss/cui-test-value-objects --base main \
...
```bash
gh pr checks <pr#> --repo cuioss/cui-test-value-objects --watch
```
...
- Every review comment MUST get a reply and MUST be resolved — fix it and say so, or explain
  ...
  ```bash
  gh api --paginate repos/cuioss/cui-test-value-objects/pulls/<pr#>/comments
  ```
- **Unresolved review threads block the merge.** A PR can show every check green and still
  ...
  ```bash
  gh api graphql --paginate -f query='query($endCursor:String){repository(owner:"cuioss",name:"cui-test-value-objects"){pullRequest(number:<pr#>){reviewThreads(first:100,after:$endCursor){pageInfo{hasNextPage endCursor} nodes{id isResolved}}}}}'
...
```bash
gh pr merge <pr#> --repo cuioss/cui-test-value-objects --squash --delete-branch
```
...
```bash
gh run list --repo cuioss/cui-test-value-objects --workflow "Release" --limit 3 \
  --json status,conclusion,displayTitle,databaseId
gh run watch <databaseId> --repo cuioss/cui-test-value-objects
```

### Step 11 — Verify the release landed

```bash
gh release view <version> --repo cuioss/cui-test-value-objects --json tagName,name,isDraft,createdAt
git fetch --tags && git tag --list <version>
curl -sfI https://repo1.maven.org/maven2/de/cuioss/test/cui-test-value-objects/<version>/cui-test-value-objects-<version>.pom \
...
```bash
mkdir -p .plan/temp
gh release view <version> --repo cuioss/cui-test-value-objects --json body --jq .body \
  > .plan/temp/release-<version>-orig.md
# ...build the reformatted body in .plan/temp/release-<version>.md...
gh release edit <version> --repo cuioss/cui-test-value-objects --notes-file .plan/temp/release-<version>.md
```
