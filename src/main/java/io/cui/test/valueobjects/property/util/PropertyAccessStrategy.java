...
import io.cui.test.valueobjects.property.impl.BuilderMetadata.BuilderMetadataBuilder;
...
public enum PropertyAccessStrategy {
    ...
    /**
     * Reads and writes property according to the JavaBean-Spec. It uses {@link PropertyUtil} to do
     * so. It acts as part of JUnit testing, therefore it will translate many of the more technical
     * Exceptions to corresponding {@link AssertionError}
     *
     * @author Oliver Wolff
     */
    BEAN_PROPERTY {
        ...
        @Override
        public Object writeProperty(final Object target,
        ...
        @Override
        public Object readProperty(final Object target,
                final PropertyMetadata propertyMetadata) {
            assertNotNull(target, TARGET_MUST_NOT_BE_NULL);
            assertNotNull(target, PROPERTY_METADATA_MUST_NOT_BE_NULL);
            try {
     ...
     * </pre>
     *
     * This strategy writes the property using <em>both</em> methods. Therefore it uses
    ...
    BUILDER_COLLECTION_AND_SINGLE_ELEMENT {
        ...
        public Object writeProperty(final Object target,
            ...
            try {
                ...
                final var writeCollectionMethod =
                    determineCollectionWriteMethod(target, propertyMetadata, builderMetadata);
     ...
     * {@link BuilderMetadata#getBuilderAddMethodName()} to
     * determine the correct write method. The parameter type is exactly the same as defined at
     * {@link PropertyMetadata#getPropertyClass()}. The read method delegates to
     * {@link PropertyAccessStrategy#BEAN_PROPERTY} because it
     * can not be read from an actual builder but from the later created bean.
     *
     * @author Oliver Wolff
     */
    ...
    /**
     * This strategy is for cases where {@link PropertyUtil} is not capable of writing an
     * attribute. This is usually the case for elements that defined a fluent-api (not void as
     * return value). The read method delegates to {@link PropertyAccessStrategy#BEAN_PROPERTY}
     */
    FLUENT_WRITER {

        @Override
        public Object writeProperty(Object target, PropertyMetadata propertyMetadata, Object propertyValue) {
            var writeMethod = MoreReflection.retrieveWriteMethod(target.getClass(),
                    propertyMetadata.getName(), propertyMetadata.resolveActualClass());
    ...
    private static final String UNABLE_TO_READ_PROPERTY =
        "Unable to read property '%s' because of '%s'";
    private static final String TARGET_MUST_NOT_BE_NULL = "target must not be null";
    private static final String PROPERTY_METADATA_MUST_NOT_BE_NULL =
        "propertyMetadata must not be null";
    private static final String UNABLE_TO_SET_PROPERTY =
        "Unable to set property '%s' because of '%s'";
     ...
     */
    public abstract Object writeProperty(final Object target,
            final PropertyMetadata propertyMetadata,
            final Object propertyValue);
     ...
     */
    public abstract Object readProperty(final Object target,
            final PropertyMetadata propertyMetadata);
