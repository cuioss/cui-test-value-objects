...
public class BeanPropertyContractImpl<T> implements TestContract<T> {
    ...
    private void checkGetterAndSetterContract() {
        ...
        if (readWriteProperties.isEmpty()) {
            ...
            /*~~(TODO: INFO needs LogRecord. Suppress: // cui-rewrite:disable CuiLogRecordPatternRecipe)~~>*/LOGGER.info("Verifying properties that are Read and Write: "
    ...
    public static final <T> Optional<TestContract<T>> createBeanPropertyTestContract(final Class<T> beanType,
        ...
        if (initialPropertyMetadata.isEmpty()) {
            /*~~(TODO: WARN needs LogRecord. Suppress: // cui-rewrite:disable CuiLogRecordPatternRecipe)~~>*/LOGGER.warn("No configured properties found to be tested, offending class: " + annotated);
            return Optional.empty();
        }
